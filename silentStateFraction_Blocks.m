...
sgtitle('Fraction of time bins as silent state in each block', 'Interpreter','latex', 'FontSize', 20)
ax1 = subplot(3, 3, [1:3]);
plot(silentStateFraction(1, :), 'Marker', '*', 'LineStyle', ':', 'Color', 'Red')
grid on
box on
...
ax2 = subplot(3, 3, [4:6]);
plot(silentStateFraction(2, :), 'Marker', '*', 'LineStyle', ':', 'Color', 'Blue')
grid on
box on
% title(['Mode Index = ', num2str(x), ' and $w_{\alpha} = $', num2str(w_input(x))], 'Interpreter','latex', 'FontSize', 20)
% xlabel('time bin in repeat sequence', 'Interpreter','latex', 'FontSize', 14)
ylabel('Silent state fraction', 'Interpreter','latex', 'FontSize', 14)
legend('Random stimulus block (60 seconds)', 'Interpreter','latex', 'FontSize', 12)


ax3 = subplot(3, 3, [7:9]);
plot(silentStateFraction(3, :), 'Marker', '*', 'LineStyle', ':', 'Color', 'Black')
grid on
box on
% title(['Mode Index = ', num2str(x), ' and $w_{\alpha} = $', num2str(w_input(x))], 'Interpreter','latex', 'FontSize', 20)
...
filenameString = ['Ani_BinaryMixModel_SilentStateFraction_per_block']
...
title('Fraction of time bins as silent state in each block', 'Interpreter','latex', 'FontSize', 20);
plot(transpose(silentStateFraction), 'Marker', '*', 'LineStyle', ':');
...
filenameString = ['Ani_BinaryMixModel_SilentStateFraction_per_block_overlaid']
