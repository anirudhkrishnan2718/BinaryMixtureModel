...
%% load results of EM algo for nModes single value (here n = 25)
...
plot(transpose(sort(w_all, 2, 'descend')), 'Marker', '*', 'LineStyle', ':')

title('Rank ordered weights of all modes', 'Interpreter','latex', 'FontSize', 20)
...
print(f1, 'Ani_BinaryMixModel_RankOrdered_ModeWeights', '-dpdf', '-fillpage');
...
axis([0 348 0 90])
title('Dominant mode at each timebin in repeated sequence trials (as colormap)', 'Interpreter','latex', 'FontSize', 20)
...
print(f2, 'Ani_BinaryMixModel_AllModes', '-dpdf', '-fillpage');

% each individual mode as a subplot

f3 = figure;
sgtitle('Binary colormap for individual modes for each timebin in repeated sequence trials', 'Interpreter','latex', 'FontSize', 20)
hold on
grid off
box on
colormap gray
...
print(f3, 'Ani_BinaryMixModel_IndividualMode_Overview', '-dpdf', '-fillpage');
...
for x = 1:1:nModes
    ...
    filenameString = ['Ani_BinaryMixModel_IndividualMode_', num2str(x)]
...
box on
colormap jet
h2 = plot(mode_emp_prob.');

title('Empirical probability of each mode across repeated sequence trials', 'Interpreter','latex', 'FontSize', 20)
xlabel('time bin in repeat sequence', 'Interpreter','latex', 'FontSize', 14)
ylabel('probability across all trials', 'Interpreter','latex', 'FontSize', 14)

set(f4,'PaperPositionMode','auto');         
set(f4,'PaperOrientation','landscape');
set(f4, 'PaperType', 'usletter');
print(f4, 'Ani_BinaryMixModel_AllModes_Probability', '-dpdf', '-fillpage');
