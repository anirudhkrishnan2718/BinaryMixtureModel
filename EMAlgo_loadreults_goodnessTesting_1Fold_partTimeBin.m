...
load('TrainTestSets_RandomPermute_singleVal_35_secondHalf.mat', 'test_set_1');

sample_in_unshuffled = test_set_1;
...
title({['Comparison of model probabilities to empirical probabilites (second half trials)'] ; ['for each unique population response (200 cells, 35 modes)'];[ '$\chi^{2} = $', num2str(chi_sq_result)]}, 'Interpreter','latex', 'FontSize', 20)
...
print(f1, 'Ani_BinaryMixModel_200Cell_35_Mode_Goodness_model_prob_secondHalf', '-dpdf', '-fillpage');
...
title({'Comparison of model to empirical first moments (200 cells, 35 modes) (second half trials) $ \sigma_{i} $', ['$ \chi^{2} =  $ ', num2str(chi_sq_result_fm)]}, 'Interpreter','latex', 'FontSize', 20)
...
print(f2, 'Ani_BinaryMixModel_200Cell_35_Mode_Goodness_firstMoment_secondHalf', '-dpdf', '-fillpage');
...
title({'Comparison of model  to empirical second moments (200 cells, 35 modes) (second half trials) $ \sigma_{i}\sigma_{j}$', ['$ \chi^{2} =  $ ', num2str(chi_sq_result_sm)]}, 'Interpreter','latex', 'FontSize', 20)
...
print(f3, 'Ani_BinaryMixModel_200Cell_35_Mode_Goodness_secondMoment_secondHalf', '-dpdf', '-fillpage');


%% spikes per timebin histogram using synthetic data set : calculation

nTimeBins_test = (timeBins / n_chunks);
...
title('Population spike count distribution synthetic data vs test data set (200 cells, 35 modes) (second half trials)', 'Interpreter','latex', 'FontSize', 20)
...
print(f4, 'Ani_BinaryMixModel_200Cell_35_Mode_Goodness_SpikesPerTimebin_secondHalf', '-dpdf', '-fillpage');
