...
for m = 1:1:num_reps
    ...
    current_full_block = binarized_spikes(:, all_times_frames(2, m) - 1391 : all_times_frames(2, m));
    silentStateFraction(1, m) = find_Silent_State_Fraction(current_full_block);
    
    current_random_block = binarized_spikes(:, all_times_frames(1, m) - 1045 : all_times_frames(1, m));
    silentStateFraction(2, m) = find_Silent_State_Fraction(current_random_block);
    
    current_repeat_block = binarized_spikes(:, all_times_frames(1, m) : all_times_frames(1, m) + 346);
    silentStateFraction(3, m) = find_Silent_State_Fraction(current_repeat_block);
...
%% plotting double y axis comparing silent state fraction to average firing rate

f4 = figure;
grid on
box on
hold on
title({'Comparing Silent state fraction and Average Cell Firing Rate in each block ', '(Full block)'}, 'Interpreter','latex', 'FontSize', 20)

yyaxis left

plot(averageFiringRate(1, :) .* frames_per_sec, 'Marker', '*', 'LineStyle', ':')
xlabel('time bin in repeat sequence', 'Interpreter','latex', 'FontSize', 14)
ylabel('Average Cell Firing Rate (spikes per second)', 'Interpreter','latex', 'FontSize', 14)

yyaxis right

plot(silentStateFraction(1, :) .* frames_per_sec, 'Marker', '*', 'LineStyle', ':')
% xlabel('time bin in repeat sequence', 'Interpreter','latex', 'FontSize', 14)
ylabel('Fraction of timebins Silent state', 'Interpreter','latex', 'FontSize', 14)

filenameString = ['Ani_BinaryMixModel_AverageFiringRate_vs_silentStateFraction_full'];
set(f4,'PaperPositionMode','auto');         
set(f4,'PaperOrientation','landscape');
set(f4, 'PaperType', 'usletter');
print(f4, filenameString, '-dpdf', '-fillpage');
...
box on
hold on
title({'Comparing Silent state fraction and Average Cell Firing Rate in each block ', '(Random stimulus block)'}, 'Interpreter','latex', 'FontSize', 20)

yyaxis left

plot(averageFiringRate(2, :) .* frames_per_sec, 'Marker', '*', 'LineStyle', ':')
xlabel('time bin in repeat sequence', 'Interpreter','latex', 'FontSize', 14)
ylabel('Average Cell Firing Rate (spikes per second)', 'Interpreter','latex', 'FontSize', 14)

yyaxis right

plot(silentStateFraction(2, :) .* frames_per_sec, 'Marker', '*', 'LineStyle', ':')
% xlabel('time bin in repeat sequence', 'Interpreter','latex', 'FontSize', 14)
ylabel('Fraction of timebins Silent state', 'Interpreter','latex', 'FontSize', 14)

filenameString = ['Ani_BinaryMixModel_AverageFiringRate_vs_silentStateFraction_rand'];
set(f5,'PaperPositionMode','auto');         
set(f5,'PaperOrientation','landscape');
set(f5, 'PaperType', 'usletter');
print(f5, filenameString, '-dpdf', '-fillpage');
...
box on
hold on
title({'Comparing Silent state fraction and Average Cell Firing Rate in each block ', '(Repeat Stimulus block)'}, 'Interpreter','latex', 'FontSize', 20)

yyaxis left

plot(averageFiringRate(3, :) .* frames_per_sec, 'Marker', '*', 'LineStyle', ':')
xlabel('time bin in repeat sequence', 'Interpreter','latex', 'FontSize', 14)
ylabel('Average Cell Firing Rate (spikes per second)', 'Interpreter','latex', 'FontSize', 14)

yyaxis right

plot(silentStateFraction(3, :) .* frames_per_sec, 'Marker', '*', 'LineStyle', ':')
% xlabel('time bin in repeat sequence', 'Interpreter','latex', 'FontSize', 14)
ylabel('Fraction of timebins Silent state', 'Interpreter','latex', 'FontSize', 14)

filenameString = ['Ani_BinaryMixModel_AverageFiringRate_vs_silentStateFraction_repeat'];
set(f6,'PaperPositionMode','auto');         
set(f6,'PaperOrientation','landscape');
set(f6, 'PaperType', 'usletter');
print(f6, filenameString, '-dpdf', '-fillpage');