...
EMResults = load('CommonIC_15_Folds_1_EMIter_500_log_lik_vs_nModes_singleVal_35_secondHalf.mat');
...
all_raw_F = load('TrainTestSets_RandomPermute_singleVal_35_secondHalf.mat');

all_cells = all_raw_F.test_set_1;

for m = 1:1:num_reps

        current_start = all_times_frames(1, m + startTrial) - (startFrame - 1);
...
title('Rank ordered weights of all modes (200 cells 35 modes)  (second half trials)', 'Interpreter','latex', 'FontSize', 20)
...
print(f1, 'Ani_BinaryMixModel_RankOrdered_ModeWeights_200Cells_35Modes_secondHalf', '-dpdf', '-fillpage');
...
axis([0 348 0 num_reps])
title('Dominant mode at each timebin in repeated sequence trials (as colormap) (200 cells 35 modes)  (second half trials)', 'Interpreter','latex', 'FontSize', 20)
...
print(f2, 'Ani_BinaryMixModel_AllModes_200Cells_35Modes_secondHalf', '-dpdf', '-fillpage');

%% each individual mode as a subplot

f3 = figure;
sgtitle('Binary colormap for individual modes for each timebin in repeated sequence trials (200 cells 35 modes)  (second half trials)', 'Interpreter','latex', 'FontSize', 20)
hold on
grid off
box on
colormap gray
...
print(f3, 'Ani_BinaryMixModel_IndividualMode_Overview_200Cells_35Modes_secondHalf', '-dpdf', '-fillpage');
...
for x = 1:1:nModes
    ...
    title(['Mode Index = ', num2str(x), ' and $w_{\alpha} = $', num2str(w_input(x)), ' (second half trials)'], 'Interpreter','latex', 'FontSize', 20)
    xlabel('time bin in repeat sequence', 'Interpreter','latex', 'FontSize', 14)
    ylabel('Mode Firing Rate', 'Interpreter','latex', 'FontSize', 14)
    
    filenameString = ['Ani_BinaryMixModel_IndividualMode_200Cells_35Modes_secondHalf_', num2str(x)]
...
title('Firing Rate of each mode across repeated sequence trials (200 cells 35 modes) (second half trials)', 'Interpreter','latex', 'FontSize', 20)
xlabel('time bin in repeat sequence', 'Interpreter','latex', 'FontSize', 14)
...
print(f4, 'Ani_BinaryMixModel_AllModes_Probability_200Cells_35Modes_secondHalf', '-dpdf', '-fillpage');
